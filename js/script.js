...
******************/
$(function () {
	var pause = 1; // will only process code within delay(function() { ... }) every 100ms.
...
jQuery(document).ready(function ($) {
	...
	$('#light').on('hover', function () {
		$('#lightText').fadeToggle('slow');
	});
	$('#pencil').on('hover', function () {
		$('#pencilText').fadeToggle('slow');
	});
	$('#cogs').on('hover', function () {
		$('#cogsText').fadeToggle('slow');
	});
	$('#globe').on('hover', function () {
		$('#globeText').fadeToggle('slow');
	});
	$('#revealRocket1').on('hover', function () {
		$('#revealRocketText').fadeToggle('slow');
	});
	setTimeout(function () {
		var typed = new Typed('#web', {
			strings: ["Web Design", "Web Development", "Digital Marketing", "Digital Content", "SEO", "Social Media Marketing", "Brand Logos"],
			typeSpeed: 20,
			loop: true,
			loopCount: 3,
			showCursor: false,
	...
	function myFunction() {
		if (document.body.scrollTop > 750 || document.documentElement.scrollTop > 750) {
			...
			$('#logo').attr("src", "images/logoDarkFill.svg");
		}
	}
});
